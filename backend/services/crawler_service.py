...
class CrawlerService:
    ...
    async def start_crawl(self, job_id: str, domain: str, product_lines: List[str], manufacturer_name: str, sharepoint_folder: str):
        ...
        try:
            ...
            # Crawl the domain
            pdf_links = await self._crawl_domain(job_id, domain, product_lines, max_pages=60)
    ...
    async def _crawl_domain(self, job_id: str, domain: str, product_lines: List[str], max_pages: int = 500) -> Set[str]:
    ...
    async def _upload_to_sharepoint(self, job_id: str, sharepoint_folder: str):
        """Upload ONLY submittal technical data PDFs to SharePoint
        
        STRICT FILTERING: Only uploads PDFs that passed the classifier's strict rules:
        - Must have matched allowed keywords (submittal data, technical data, data sheet, etc.)
        - Must NOT have matched any disallowed keywords (installation, warranty, spec, etc.)
        """
        # Get only approved submittal data sheets for this job
        # is_technical=True means it passed ALL strict classifier rules
    ...
    async def _download_with_playwright(self, url: str) -> bytes:
        """Download a PDF using Playwright's request API for sites that block direct requests"""
        try:
            async with async_playwright() as p:
                ...
                # Use the request API which inherits browser context
                ...
                await browser.close()
                logger.warning(f"Playwright download failed: HTTP {response.status}")
                return None
